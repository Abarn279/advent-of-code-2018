...
class Unit:
    def __init__(self, pos: Vector2, team: Team, ad: int):
        self.pos = pos; 
        self.team = team
        self.id = uuid.uuid1()
        self.hp = 200
...
class GameManager:
    def __init__(self, grid, elf_attack_power):
        self.grid = grid
        ...
        for y in range(len(self.grid)):
            for x in range(len(self.grid[y])):
                if self.grid[y][x] == 'G':
                    self.units.append(Unit(Vector2(x, y), Team.GOBLINS, 3))
                elif self.grid[y][x] == 'E':
                    self.units.append(Unit(Vector2(x, y), Team.ELVES, elf_attack_power))
                    self.elfcount += 1
    ...
    def all_elves_alive(self):
        return len([i for i in self.units if i.team == Team.ELVES]) == self.elfcount
...
while True:
    ...
    if (game_manager.all_elves_alive()):
        print((game_manager.rounds_completed) * game_manager.get_combined_health())
        print(game_manager.rounds_completed)
        print(game_manager.get_combined_health())
        break
