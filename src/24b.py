...
class Team:
    ...
    def give_boost(self, boost):
        for i in self.groups: i.boost(boost)
    ...
    def do_target_selection(self, enemy_team):
        ...
        able_to_select = []
        for group in groups:
            able_to_select.append(group.do_target_selection(enemy_team.groups))
        return True if all(able_to_select) else False
...
class GameManager:
    def __init__(self, team1, team2, team1_boost):
        self.immune_system = team1
        self.immune_system.give_boost(team1_boost)
        self.infection = team2

    def __do_target_selection(self):
        can1 = self.immune_system.do_target_selection(self.infection)
        can2 = self.infection.do_target_selection(self.immune_system)
    ...
    def do_fight(self):
        if not self.__do_target_selection():
            return False
...
for boost in range(0, 10000000):
    ...
    gm = GameManager(Team(immune_system_lst[:]), Team(infection_lst[:]), boost)

    while not gm.game_over():
        if not gm.do_fight():
            break

    if not gm.immune_system.is_all_dead() and gm.infection.is_all_dead():
